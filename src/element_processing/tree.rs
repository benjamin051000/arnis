...
pub fn create_tree(editor: &mut WorldEditor, (x, y, z): (i32, i32, i32), tree_type: u8, snow: bool) {
    ...
    match tree_type {
        1 => {
            ...
            for j in (3..=8).rev() {
                round(editor, OAK_LEAVES, x, y + j, z, &ROUND1_COORDS);
            }

            for j in (4..=7).rev() {
                round(editor, OAK_LEAVES, x, y + j, z, &ROUND2_COORDS);
            }

            for j in (5..=6).rev() {
                round(editor, OAK_LEAVES, x, y + j, z, &ROUND3_COORDS);
            }
            ...
            if snow {
                ...
                for j in (6..=9).rev() {
                    round(editor, SNOW_LAYER, x, y + j, z, &ROUND1_COORDS);
                }

                for j in (5..=8).rev() {
                    round(editor, SNOW_LAYER, x, y + j, z, &ROUND2_COORDS);
                }

                for j in (6..=7).rev() {
                    round(editor, SNOW_LAYER, x, y + j, z, &ROUND3_COORDS);
                }
            }
        }
        2 => {
            ...
            for j in [9, 7, 6, 4, 3] {
                round(editor, BIRCH_LEAVES, x, y + j, z, &ROUND1_COORDS);
            }

            for j in [6, 3] {
                round(editor, BIRCH_LEAVES, x, y + j, z, &ROUND2_COORDS);
            }
